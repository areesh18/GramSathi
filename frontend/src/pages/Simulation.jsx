    ...
    <div className="min-h-screen bg-gray-50 flex flex-col relative font-sans">
      ...
      {step === 1 && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
          <div className="mb-8 p-4 bg-yellow-100 text-yellow-800 rounded-xl border border-yellow-300 shadow-sm">
            👇 <span className="font-bold">Task:</span> Tap the button below to open the camera.
          </div>
      ...
      {step === 2 && (
        <div className="absolute inset-0 bg-black z-50 flex flex-col items-center justify-center">
          ...
          <button onClick={() => setStep(1)} className="mt-12 text-white opacity-70">Cancel</button>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="p-6 animate-in slide-in-from-bottom duration-500">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center mb-6">
             ...
             <h2 className="font-bold text-xl text-gray-800">Raju Kirana Store</h2>
             <p className="text-gray-500 text-sm">raju@upi • Verified Merchant</p>
          </div>

          <div className="text-center mb-8">
             <label className="text-gray-500 text-sm font-medium">Enter Amount</label>
          ...
          <button 
            disabled={!amount}
            onClick={() => setStep(4)}
            className={`w-full py-4 rounded-xl font-bold text-white text-lg shadow-lg transition-all active:scale-95 ${amount ? 'bg-blue-600' : 'bg-gray-300 cursor-not-allowed'}`}
          >
            Proceed to Pay
          </button>
        </div>
        ...
        <div className="fixed inset-0 bg-white z-50 flex flex-col animate-in slide-in-from-bottom duration-300">
           <div className="p-6 bg-gray-50 flex-1 flex flex-col items-center justify-center">
             <div className="bg-white p-4 rounded-xl shadow-sm mb-6 flex items-center gap-3">
                ...
                <span className="font-bold text-gray-700">State Bank of India ****8892</span>
             </div>
             <h3 className="font-bold text-lg mb-8 text-gray-800">Enter 4-Digit UPI PIN</h3>
             
             <div className="flex gap-4 mb-8">
               {[...Array(4)].map((_, i) => (
                 <div key={i} className={`w-4 h-4 rounded-full transition-colors duration-200 ${pin.length > i ? 'bg-black scale-125' : 'border-2 border-gray-300'}`}></div>
               ))}
             </div>
           </div>
      ...
      {step === 5 && (
        <div className="fixed inset-0 bg-green-600 z-50 flex flex-col items-center justify-center text-white p-6 animate-in zoom-in duration-300">
          ...
          <p className="opacity-90 mb-12 text-green-100">Transaction ID: #839201</p>
          
          <div className="bg-green-700/50 p-4 rounded-xl w-full max-w-sm mb-8 border border-green-500/30 backdrop-blur-md">
            <div className="flex justify-between items-center mb-2">
               <span className="text-sm opacity-90 font-medium">Reward Earned</span>
               <span className="font-bold text-yellow-300 text-lg">★ 50 pts</span>
            </div>
