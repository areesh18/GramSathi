...
import { Bell, MapPin, Shield, Smartphone, Trophy, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="pb-24 md:pb-8 min-h-screen bg-gray-50">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-4 md:items-center">
            ...
            <div className="flex-1 relative max-w-md">
              ...
              />
              <span className="absolute right-3 top-3 bg-blue-100 text-blue-700 p-1 rounded-lg text-xs font-bold cursor-pointer hover:scale-105 transition">🎤</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-8 -mt-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h2 className="font-bold text-gray-800 mb-4 text-lg flex items-center gap-2">
              <span className="bg-yellow-100 text-yellow-700 p-1 rounded">🔥</span> Daily Challenge
            </h2>
            <Link to="/simulation/upi">
              <div className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md border border-blue-100 relative overflow-hidden group transition-all duration-300 transform hover:-translate-y-1 cursor-pointer h-full">
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h4 className="font-bold text-gray-800 text-xl">Practice UPI Payment</h4>
                    <p className="text-sm text-gray-500 mt-1 leading-relaxed">Learn to scan & pay safely in a risk-free demo environment.</p>
          ...
          <div>
             <h2 className="font-bold text-gray-800 mb-4 text-lg flex items-center gap-2">
              <span className="bg-green-100 text-green-700 p-1 rounded">📢</span> Update
            </h2>
            <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl p-6 text-white shadow-md flex flex-col justify-center h-full relative overflow-hidden">
                ...
                <h3 className="font-bold text-xl relative z-10">Kisan Samman Nidhi</h3>
                <p className="text-purple-100 text-sm mt-2 relative z-10">Next installment of ₹2,000 is arriving next week.</p>
        ...
        {/* Quick Actions Grid */}
        <div className=''>
            <h2 className=" font-bold text-gray-800 mb-4 text-lg">Quick Services</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { title: "Gov Schemes", icon: <Shield size={24} />, color: "bg-green-100 text-green-700" },
                  { title: "Find Doctor", icon: "🏥", color: "bg-orange-100 text-orange-700", isTextIcon: true },
                  { title: "Mandi Prices", icon: "💰", color: "bg-yellow-100 text-yellow-700", isTextIcon: true },
                  { title: "Land Records", icon: "📜", color: "bg-teal-100 text-teal-700", isTextIcon: true },
                ].map((item, idx) => (
                  <div key={idx} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center gap-3 hover:shadow-md hover:border-blue-200 transition cursor-pointer group">
                      <div className={`w-14 h-14 rounded-full flex items-center justify-center ${item.color} group-hover:scale-110 transition-transform`}>
                          {item.isTextIcon ? <span className="text-2xl">{item.icon}</span> : item.icon}
                      </div>
                      <span className="font-semibold text-gray-700 group-hover:text-blue-700 transition-colors">{item.title}</span>
                  </div>
                ))}
            </div>
        </div>
